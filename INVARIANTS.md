...
- An action needing a signed-in session is named `auth-`, one running signed out `pub-`, a site with a login carries `whoami.tcl`, and every `auth-` run reports the identity it saw: a caller cannot otherwise tell an empty result from an unopened page, and records the absence as a finding. Elaboration: `SESSION-CONTRACT.md`.
