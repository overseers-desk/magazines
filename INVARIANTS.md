...
- An action needing a signed-in session is named `auth-`, one running signed out `pub-`, and a site with a login carries `whoami.tcl`: a caller cannot otherwise tell an empty result from an unopened page, and records the absence as a finding. Elaboration: `SESSION-CONTRACT.md`.
