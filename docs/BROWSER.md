...
**D6. The serialiser harness owns the CDP browser lifecycle; the skill is a pure websocket peer.** Resolves problem 6. The harness launches the browser, waits for a page target, drives the skill over that websocket through the `cdp` module, and reaps the browser on every exit path. Because a persistent browser reparents when its launcher returns (`chromium-process-model.md`), the reap finds it by a stable attribute, its `--remote-debugging-port` string, rather than by a PID captured at launch: SIGTERM to let it flush, then SIGKILL, then removal of any Singleton symlinks a SIGKILL leaves pointing at a dead PID. A skill therefore holds no browser PID and never launches a browser of its own; the policed verbs read the websocket and drive the page. Recorded in commit `cd951b0`.

**D7. The harness opens the last-used profile within the user-data-dir; it never selects a profile.** `browser-serialiser` launches with `--user-data-dir` only, never `--profile-directory`, so Chromium opens whichever profile was last active. This is the contract: the user chooses which account the agent gets by making that account's profile the last-used one before handing off, and switches account the same way. It exists because one profile cannot hold two accounts of the same site and different tasks need different accounts; if the harness pinned a profile, the user would be forced to log out and back in between tasks, incurring OTP friction and a "new device / rapid re-login" signal that risks a ban. Consequences for anyone extending any browser skill: do not add profile-selection logic, do not read or set `--profile-directory`, and never ask the user how the profile is to be chosen. Profile choice is the user's, made outside the skill by last-use; the harness is blind to the multi-profile feature by design. This is settled — it is not an open question to raise with the user again.
...
**API / non-browser skills** are not affected by D1-D5 because they do not touch a browser. Examples: serpapi, renfe.com, claude-api, send-email, courier. Credentials live in environment variables or `~/.config/magazines/config.ini`.
