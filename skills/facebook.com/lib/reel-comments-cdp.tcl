# The facebook.com reel-comments driver. A reel's comments load lazily over
# GraphQL inside the authenticated viewer, so there is no single-dump path: this
# clicks through the viewer, harvests the GraphQL responses, and writes a
# synthetic HTML document, one block per top-level comment.
#
# fbcdp::sv_fetch is the policed-surface entry the actions call; fbcdp::fetch is
# the raw-CDP one the direct-tclsh path uses.
...
source [file join [file dirname [info script]] fb-common.tcl]
