...
#
# Serialiser path (see SKILL.md): browser-serialiser facebook.com/auth-parse-activity <profile-id|activity-url> [--max-rounds N]
#   navigates to the activity log, scrolls to lazy-load entries, dumps the DOM, parses.
# Direct path (legacy, file-fed): tclsh auth-parse-activity.tcl <html-file>
#
...
if {[info exists argv0] && [file tail [info script]] eq [file tail $argv0]} {
    if {[llength $argv] < 1} {
        puts "Usage: auth-parse-activity.tcl <activity.html>"
        exit 1
    }
