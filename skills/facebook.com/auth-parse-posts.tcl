...
#
# Serialiser path (see SKILL.md §5): browser-serialiser facebook.com/auth-parse-posts <handle|profile-url> [--owner-id ID]
#   navigates to the profile, dumps the rendered DOM, and runs the identical parse.
# Direct path (legacy, file-fed): tclsh auth-parse-posts.tcl <html-file> [--owner-id ID]
#
...
#     browser-serialiser facebook.com/auth-parse-posts <handle|profile-url> [--owner-id ID]
# ---------------------------------------------------------------------------
...
if {[info exists argv0] && [file tail [info script]] eq [file tail $argv0]} {
    ...
    if {[llength $rest] < 1} {
        puts "Usage: auth-parse-posts.tcl <profile.html> \[--owner-id ID\]"
        exit 1
    }
