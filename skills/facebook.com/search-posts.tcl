...
#
# Serialiser path (see SKILL.md): browser-serialiser facebook.com/search-posts ...
# Direct path (file-fed): tclsh search-posts.tcl <bodies-file> keyword ...
#
...
proc serialiser_run {skillArgs} {
    ...
    if {$group eq "" && ![llength $terms]} {
        emit "Usage: facebook.com/search-posts \[--group <id|name|url>\] \[--feed\] \[--max-rounds N\] <terms...>"
        return
    }
...
if {[info exists argv0] && [file tail [info script]] eq [file tail $argv0]} {
    if {[llength $argv] < 1} {
        puts "Usage: search-posts.tcl <bodies-file> \[keyword ...\]"
        exit 1
    }
