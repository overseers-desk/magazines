...
#
# Usage:
#   browser-serialiser facebook.com/auth-reel-comments-cdp URL [--max-rounds N]
#   tclsh auth-reel-comments-cdp.tcl URL [--out PATH] [--bodies-json PATH] [--debug]
...
if {[info exists argv0] && [file tail [info script]] eq [file tail $argv0]} {
    ...
    if {[llength $positional] != 1} {
        puts stderr "Usage: auth-reel-comments-cdp.tcl URL \[--out PATH\] \[--bodies-json PATH\] \[--max-rounds N\] \[--debug\]"
        exit 1
    }
    set url [lindex $positional 0]

    if {![info exists ::env(CDP_WS_URL)] || $::env(CDP_WS_URL) eq ""} {
        puts stderr "ERROR: CDP_WS_URL not set; run via: browser-serialiser facebook.com/auth-reel-comments-cdp <reel-url> \[--out FILE\]"
        exit 1
    }
