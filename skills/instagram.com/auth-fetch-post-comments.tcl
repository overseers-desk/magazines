...
#     browser-serialiser instagram.com/auth-fetch-post-comments comments <shortcode|post_id|media_id> [--limit N]
#
...
proc main {} {
    ...
    if {![llength $argv] || [lindex $argv 0] ne "comments"} {
        puts "Usage: auth-fetch-post-comments.tcl comments <shortcode|post_id|media_id> \[--limit N\]"
        exit 1
    }
    ...
    if {![info exists ::env(CDP_WS_URL)] || $::env(CDP_WS_URL) eq ""} {
        puts stderr "ERROR: CDP_WS_URL not set; run via: browser-serialiser instagram.com/auth-fetch-post-comments ..."
        exit 1
    }
