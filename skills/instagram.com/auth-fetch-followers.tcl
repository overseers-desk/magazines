...
proc main {} {
    global argv
    if {![llength $argv]} {
        puts "Usage: auth-fetch-followers.tcl followers|following <handle> \[--limit N\] \[--csv PATH\]"
        exit 1
    }
    ...
    if {![info exists ::env(CDP_WS_URL)] || $::env(CDP_WS_URL) eq ""} {
        puts stderr "ERROR: CDP_WS_URL not set; run via: browser-serialiser instagram.com/auth-fetch-followers ..."
        exit 1
    }
