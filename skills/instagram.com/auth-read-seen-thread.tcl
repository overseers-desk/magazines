...
# exists alongside `auth-inbox-noninvasive.tcl` because reading a thread's message
...
proc main {} {
    global argv
    if {![llength $argv]} {
        puts "Usage: auth-read-seen-thread.tcl thread|by-handle|all-seen ..."
        exit 1
    }
    ...
    if {![info exists ::env(CDP_WS_URL)] || $::env(CDP_WS_URL) eq ""} {
        puts stderr "ERROR: CDP_WS_URL not set; run via: browser-serialiser instagram.com/auth-read-seen-thread ..."
        exit 1
    }
