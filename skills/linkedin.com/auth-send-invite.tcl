...
#
# Serialiser path (see SKILL.md): browser-serialiser linkedin.com/auth-send-invite VANITY_NAME "Note" [--dry-run]
...
# Direct path (legacy, CDP client): CDP_WS_URL from the harness/overseer relay; browser-serialiser linkedin.com/auth-send-invite.
#
# Usage:
#     browser-serialiser linkedin.com/auth-send-invite VANITY_NAME "Note text (<=300 chars)"
#     browser-serialiser linkedin.com/auth-send-invite VANITY_NAME "Note text" --dry-run   # stop before clicking Send
...
proc main {} {
    ...
    if {[llength $positional] < 1} {
        puts stderr "Usage: auth-send-invite.tcl VANITY_NAME \[note\] \[--dry-run\]"
        exit 2
    }
    ...
    if {![info exists ::env(CDP_WS_URL)] || $::env(CDP_WS_URL) eq ""} {
        puts stderr "ERROR: CDP_WS_URL not set; run via: browser-serialiser linkedin.com/auth-send-invite VANITY_NAME \"Note\" \[--dry-run\]"
        exit 1
    }
