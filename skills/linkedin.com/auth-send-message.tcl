...
#
# Serialiser path (see SKILL.md): browser-serialiser linkedin.com/auth-send-message VANITY_NAME "Message" [--dry-run]
...
# Direct path (legacy, CDP client): CDP_WS_URL from the harness/overseer relay; browser-serialiser linkedin.com/auth-send-message.
#
# Usage:
#     browser-serialiser linkedin.com/auth-send-message VANITY_NAME "Message text"
#     browser-serialiser linkedin.com/auth-send-message VANITY_NAME "Message text" --dry-run   # stop before clicking Send
...
proc main {} {
    ...
    if {[llength $positional] < 2} {
        puts stderr "Usage: auth-send-message.tcl VANITY_NAME \"Message text\" \[--dry-run\]"
        exit 2
    }
    ...
    if {![info exists ::env(CDP_WS_URL)] || $::env(CDP_WS_URL) eq ""} {
        puts stderr "ERROR: CDP_WS_URL not set; run via: browser-serialiser linkedin.com/auth-send-message VANITY_NAME \"Message\" \[--dry-run\]"
        exit 1
    }
...
#
#     browser-serialiser linkedin.com/auth-send-message VANITY_NAME "Message" [--dry-run]
# ---------------------------------------------------------------------------
