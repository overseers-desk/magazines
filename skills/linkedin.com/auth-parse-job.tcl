...
# Direct path (file-fed, for testing): tclsh auth-parse-job.tcl <html-file> [job-id]
#
...
proc serialiser_run {skillArgs} {
    ...
    if {$arg eq ""} {
        emit "Usage: linkedin.com/auth-parse-job <job-id-or-url>"
        return
    }
...
if {[info exists argv0] && [file tail [info script]] eq [file tail $argv0]} {
    ...
    if {[llength $argv] < 1} {
        puts "Usage: auth-parse-job.tcl <html-file> \[job-id\]"
        exit 1
    }
