...
- **The Sec-CH-UA / UA-string split is the one fingerprint hardened globally.** `--user-agent` rewrites the UA header but not the client hints, so a Chromium snap sends `Sec-CH-UA: "Chromium";v="N", "Not.A/Brand";v="8"` beside a UA claiming Chrome, with no `"Google Chrome"` brand. Instagram's private `direct_v2` API rejects that split as `useragent mismatch` (the inbox shell loads, the API call does not). That is D3 revisited for one probe: `cdp::connect` calls `matchClientHints`, which reads the live UA, derives the Chrome major, and pushes a matching `userAgentMetadata` over `Emulation.setUserAgentOverride` (CDP is the only way to set hints). Always-on for every browser skill, since the split is a general headless tell, not site-specific; other probes (software WebGL, screen size) stay per-skill residuals.
