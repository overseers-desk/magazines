...
- **The cash amount is not in any member API.** The stays, folio and reservation endpoints return points and metadata only (folio comes back empty; reservation-detail paths 404). The cash figure lives solely in the booking-confirmation email (from `tx.ihg.com`), charged in USD — fields "Nightly cash amount" / "Total credit card charge". Use the courier skill for that.
